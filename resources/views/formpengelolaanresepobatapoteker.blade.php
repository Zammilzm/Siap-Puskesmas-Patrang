...
<body style="background-color:#cfd8dc;">
	...
	<div class="container">
		<div class="row">
			<form class="col s11 offset-s2" method="POST" action="{{ url('/update/apotekers/'.$datareseps->id_resep) }}">
				{{method_field('put')}}
				{{ csrf_field() }}
				...
				<div class="row">
					<div class="input-field col s10">
						<input id="icon_telephone" type="tel" class="validate" name="resep_tersedia" value="{{$datareseps->resep_tersedia}}">
						<label for="icon_telephone">Resep Tersedia</label>
					</div>
				</div>
				<div class="row">
					<div class="col-md-7">
						<button type="submit" class="btn btn-primary btn-block" >SIMPAN</button><br>
						<a href="{{URL('resepapoteker')}}" class="btn btn-primary">
							BATAL
						</a>
					</div>
				</div>
