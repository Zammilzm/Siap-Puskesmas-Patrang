...
<body style="background-color:#cfd8dc;">
	...
	<div class="container">
		<div class="row">
			<form method="POST" action="{{ url('/update/pendaftaran/'.$dataPendaftar->id_pendaftaran) }}" class="col s11 offset-s2">
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
				{{method_field('put')}}
				{{ csrf_field() }}
				<h2 style="text-decoration:underline;">EDIT PENDAFTARAN</h2>
				<div class="row">
					<div class="input-field col s10">
						<input id="icon_telephone" type="text" class="validate" name="id_pendaftaran" value="{{$dataPendaftar->id_pendaftaran}}" disabled>
						<label for="pendaftaran">ID Pendaftaran</label>
					</div>
				</div>
				<div class="row">
					<div class="input-field col s3">
						<select name="id_pasien" disabled>
<!-- 							<option value="" disabled selected>Pilih Id Pasien</option> -->
							@foreach($polis as $poli)
							<option {{($dataPendaftar->id_poli==$poli->id_poli)?'selected':''}} 
								value="{{$poli->id_poli}}">{{$poli->id_poli}}
							</option>
							@endforeach
						</select>
				...
				<div class="row">
					<div class="input-field col s3">
						<select name="id_poli">
							...
							@foreach($polis as $poli)
							<option {{($dataPendaftar->id_poli==$poli->id_poli)?'selected':''}} 
								value="{{$poli->id_poli}}">{{$poli->id_poli}}</option>
								@endforeach
							</select>
					...
					<div class="row">
						<div class="row">
							...
							<div class="row">
								<div class="input-field col s10">
								<p>Tanggal Periksa</p>
									<input id="icon_telephone" type="date" class="validate" name="tanggal_periksa" value="{{$dataPendaftar->tanggal_periksa}}" required>
							...
							<div class="row">
								<div class="col-md-7 offset-s2">
									...
									<br>
									<a href="{{URL('pendaftaran')}}" class="btn btn-primary" >Batal</a>
								</div>
							</div>
						</form>
					</div>
